 ...
 */
public class SidCustomerRole extends Role implements Customer{
	private String name;
	 ...
	 */
	public SidCustomerRole(String name){
		super();
		this.name = name;
	}
	 ...
	 */
	public void setHost(SidHostAgent host) {
		this.host = host;
	}
		...
		timer.schedule(new TimerTask() {
			public void run() {
				if(wait == null){
					...
					host.msgCancelSeating(SidCustomerRole.this);
				...
				if (money > 5.99){
					if (money < 8.99){
						...
						if (choice.equals("Out")){
							...
							wait.msgReadytoOrder(SidCustomerRole.this);
						}
					} else {
						do {
							choice = myChoices.getChoice(hungerLevel);
