...
import restaurant_sid.gui.CookGui;
...
public class SidCookAgent extends Agent {
	SidWaiterCookMonitor orders = new SidWaiterCookMonitor(this);
	...
	public List<SidMarketAgent> myMarkets = Collections.synchronizedList(new ArrayList<SidMarketAgent>());
	...
	public SidCookAgent(String name) {
		super();
	...
	public void addMarket(SidMarketAgent m){
		myMarkets.add(m);
	}
	...
	public void msgHereIsAnOrder(Waiter w, String choice, int tNum) {
		System.out.println("Received order for " + choice);
	...
	public void msgNoDelivery(){
		...
		SidMarketAgent m = myMarkets.get(0);
		myMarkets.remove(0);
		myMarkets.add(m);
		stateChanged();
	}
	
	public void msgMarketOut(SidMarketAgent m){
		myMarkets.remove(m);
	}
	...
	protected boolean pickAndExecuteAnAction() {
		...
		if (!(orders.orders.isEmpty())){
			CookOrder(orders.orders.get(0));
			orders.orders.remove(0);
			return true;
		} else if (!completeOrders.isEmpty()){
			sendOrder();
			return true;
	...
	static public class Order{
		...
		public Order(Waiter w, String m, int t){
			myW = w;
			meal = m;
			tNum = t;
		}
